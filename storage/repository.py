"""Baseline and scan data-access layer for SentinelLite.
...
dependency -- it only translates between ``core`` data structures and
SQLite rows.

The scan-related functions accept plain values (strings, ints, tuples)
rather than importing ``core.scanner``'s dataclasses, to avoid a circular
import (``core.scanner`` calls into this module to persist its results).
"""
...
class ScanMetadata(NamedTuple):
    """Metadata for a stored scan, without its (potentially large) result list."""
...
class ScanResultRow(NamedTuple):
    """A single stored per-file scan outcome."""
...
def insert_scan(
    ...
    *,
    ...
    unchanged_count: int,
    error_count: int,
    results: Iterable[tuple[str, str, str | None, str | None, int | None, str | None]],
) -> int:
    """Persist a new scan and its per-file results.

    Args:
        results: An iterable of
            ``(relative_path, status, baseline_hash, current_hash, size, error_message)``
            tuples, one per compared file (see ``core.scanner.ScanResult``).
    ...
    with connection:
        ...
        connection.executemany(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            ((scan_id, *row) for row in results),
        )
...
def get_latest_scan_id(connection: sqlite3.Connection, baseline_id: int | None = None) -> int | None:
    ...
    If ``baseline_id`` is given, only scans against that baseline are considered.
    """
...
def get_scan_metadata(connection: sqlite3.Connection, scan_id: int) -> ScanMetadata | None:
    """Return a scan's metadata (without per-file results), or None if it does not exist."""
    row = connection.execute(
